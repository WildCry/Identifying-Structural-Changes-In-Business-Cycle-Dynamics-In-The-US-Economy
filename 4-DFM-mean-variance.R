...
fetch_series_data <- function(series_id) {
  fredr_series_observations(series_id = series_id) %>%
    ...
    filter(date >= as.Date("1949-01-01") & date <= as.Date("2024-02-01")) %>%
...
# Init ----
init <- c(
  p_cc = 7.318876e-12, p_dd = 8.356983e-01, p_uu = 8.529030e-01, p_mm = 8.903220e-01,
  mu_u = 4.074266e-01, mu_d = -1.336832e+00, mu_c = -6.810875e+01, mu_m = 1.150232e-01,
  # eta_u = 3.755326e+00, eta_d = 3.903624e-01, eta_c = 8.608428e-07, eta_m = 1.621134e-01,
  eta_u = 2, eta_d = 1, eta_c = 2, eta_m = 0.1,
  
  phi1 = -4.190913e-02,
  
  gamma_1.0 = 2.066038e-01,
  gamma_2.0 = 1.798277e-01,
  gamma_3.0 = 3.496063e-01,
  gamma_4.0 = 2.469622e-01,
  
  psi_1.1 = -1.302461e-01,
  psi_2.1 =  -1.133541e-01,
  psi_3.1 = -9.964002e-01,
  psi_4.1 = 1.035659e-01,
  
  sigma_1 = 6.450205e-01, sigma_2 = 7.416867e-01,
  sigma_3 = 2.802722e-06, sigma_4 = 4.574641e-01
)
...
# Set the constraints
ineqA <- matrix(0, nrow = 30, ncol = length(init), dimnames = list(NULL, names(init)))
...
ineqA[c(19, 20), "p_cc"] <- c(1, -1)
ineqA[c(21, 22), "p_mm"] <- c(1, -1)
# Up/down states must be positive/negative
ineqA[23, "mu_u"] <- 1
ineqA[24, "mu_d"] <- -1
ineqA[25, "mu_c"] <- -1
ineqA[26, "mu_m"] <- 1
diag(ineqA[c(27, 28, 29, 30), grepl("eta_", colnames(ineqA))]) <- 1
...
solve <- maxLik(
  logLik = objective, start = init, method = "BFGS",
  finalHessian = TRUE, hess = NULL,
...
g3 <- ggplot() +
  ...
  scale_y_continuous(
    name = "Value", limits = range(toplot3[variable == "dcf", ]$value, na.rm = TRUE),
    sec.axis = sec_axis(name = "Probability", ~ ((. - d_range1[1]) / diff(d_range1) * diff(p_range1) + p_range1[1]) * 100)
  ) +
  geom_ribbon(
    data = toplot3[variable %in% "pr_d", ],
...
g4 <- ggplot() +
  ...
  scale_color_manual(values = "black") +
  scale_y_continuous(
    name = "Value", limits = range(toplot4[variable %in% c("d.dcf"), ]$value, na.rm = TRUE),
    sec.axis = sec_axis(name = "Probability", ~ ((. - d_range2[1]) / diff(d_range2) * diff(p_range2) + p_range2[1]) * 100)
  ) +
  geom_ribbon(
    data = toplot4[variable %in% "pr_d", ],
...
write.csv(Ctt, "Data/4-DFM-mean-variance-factor.csv")
write.csv(prob, "Data/4-DFM-mean-variance-prob.csv")
write.csv(estimates, "estimerte parametere/4-DFM-mean-variance-estimates.csv")
write.csv(standard_errors, "estimerte parametere/4-DFM-mean-variance-standard_errors.csv")
write.csv(y_pred, "Data/4-DFM-mean-variance-predictions.csv")
...
write.csv(metrics, "estimerte parametere/4-DFM-mean-variance-metrics.csv")
