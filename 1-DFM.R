...
init <- c(
  ...
  sigma_1 = 0.437852, sigma_2 = 0.691700,
  sigma_3 = 0.368175, sigma_4 = 0.138908
...
# CONSTRAINTS
bounds_list <- list(phi1 = c(0.01,0.9),
                    mu_d = c(0,0), 
                    p_dd = c(1,1), 
                    psi_1.1 = c(-0.9,0.9),
                    psi_2.1 = c(-0.9,0.9),
...
bounds_matrix <- do.call(rbind, lapply(bounds_list, function(x) x))
bounds_matrix <- t(bounds_matrix)
...
# eqA = matrix(0, nrow = 2, ncol = length(init), dimnames = list(NULL, names(init)))
# eqA[c(1), "p_dd"] <- c(1)
# eqA[c(2), "mu_d"] <- c(1)
# 
# eqB <- matrix(c(
#   rep(-1, 2)
# ), nrow = nrow(eqA), ncol = 1)
...
# export ----
write.csv(Ctt, "Data/1-DFM-factor.csv")

write.csv(estimates, "estimerte parametere/1-DFM-estimates.csv")
write.csv(standard_errors, "estimerte parametere/1-DFM-standard_errors.csv")
write.csv(y_pred, "Data/1-DFM-predictions.csv")
...
write.csv(metrics, "estimerte parametere/1-DFM-metrics.csv")
